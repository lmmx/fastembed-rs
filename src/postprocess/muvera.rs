...
fn hamming_distance_matrix(n: usize) -> Vec<Vec<u32>> {
    ...
    for i in 0..n {
        for j in 0..n {
            matrix[i][j] = hamming_distance(i as u64, j as u64);
        }
    }
    matrix
}
    ...
    pub fn get_cluster_ids(&self, vectors: &[Vec<f32>]) -> Vec<u64> {
        vectors
            ...
            .map(|vec| {
                ...
                for d in 0..self.dim {
                    for k in 0..self.k_sim {
                        dot_products[k] += vec[d] * self.simhash_vectors[d][k];
                    }
                }
...
impl Muvera {
    ...
    pub fn process(
        ...
        for proj_idx in 0..self.r_reps {
            ...
            let mut cluster_vector_indices: Vec<Vec<usize>> =
            ...
            for cluster_idx in 0..self.num_partitions {
                if self.dim_proj < self.dim {
                    ...
                    for proj_d in 0..self.dim_proj {
                        let mut val = 0.0f32;
                        for d in 0..self.dim {
                            val += cluster_centers[cluster_idx][d] * dim_reduction[d][proj_d];
                        }
                        output.push(val * scale);
                    }
                } else {
                    // No projection needed (dim_proj == dim)
                    output.extend_from_slice(&cluster_centers[cluster_idx]);
                }
            }
        }
